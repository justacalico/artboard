#!/usr/bin/env bash
# Package a built Flutter Linux bundle as a .deb for Debian/Ubuntu.
# Installs to /opt/artboard with a /usr/bin/artboard symlink, a desktop entry
# and an icon.
#
# Usage: scripts/build-deb.sh <bundle-dir> <version> <deb-arch> <output-file>
#   deb-arch: amd64 or arm64
set -euo pipefail

BUNDLE_DIR="${1:?usage: build-deb.sh <bundle-dir> <version> <deb-arch> <output-file>}"
VERSION="${2:?usage: build-deb.sh <bundle-dir> <version> <deb-arch> <output-file>}"
ARCH="${3:?usage: build-deb.sh <bundle-dir> <version> <deb-arch> <output-file>}"
OUT="${4:?usage: build-deb.sh <bundle-dir> <version> <deb-arch> <output-file>}"

if ! command -v dpkg-deb >/dev/null 2>&1; then
  echo "build-deb: dpkg-deb not found (install the dpkg-dev package)" >&2
  exit 1
fi
if [ ! -d "$BUNDLE_DIR" ]; then
  echo "build-deb: bundle dir not found: $BUNDLE_DIR" >&2
  exit 1
fi
if [ "$ARCH" != "amd64" ] && [ "$ARCH" != "arm64" ]; then
  echo "build-deb: unsupported arch: $ARCH" >&2
  exit 1
fi

# The icon ships in the repo, not in the bundle; scripts run from the repo root.
ICON="linux/runner/artboard_icon.png"

PKGDIR="$(mktemp -d)"
trap 'rm -rf "$PKGDIR"' EXIT
mkdir -p "$PKGDIR/opt/artboard" "$PKGDIR/usr/bin" "$PKGDIR/DEBIAN" \
  "$PKGDIR/usr/share/applications" \
  "$PKGDIR/usr/share/icons/hicolor/256x256/apps"

cp -r "$BUNDLE_DIR/." "$PKGDIR/opt/artboard/"
ln -sf /opt/artboard/artboard "$PKGDIR/usr/bin/artboard"
install -m 644 "$ICON" \
  "$PKGDIR/usr/share/icons/hicolor/256x256/apps/artboard.png"

cat > "$PKGDIR/usr/share/applications/artboard.desktop" <<'EOF'
[Desktop Entry]
Type=Application
Name=Artboard
Comment=Draw a picture, play it as music
Exec=/usr/bin/artboard
Icon=artboard
Categories=AudioVideo;Audio;Graphics;
Terminal=false
EOF

cat > "$PKGDIR/DEBIAN/control" <<EOF
Package: artboard
Version: $VERSION
Section: utils
Priority: optional
Architecture: $ARCH
Maintainer: HttpAnimations <noreply@gitlab.com>
Description: Draw a picture, play it as music
 A drawing app where the canvas is also an instrument. A scan line sweeps
 the drawing and plays a note wherever it crosses a stroke.
EOF

dpkg-deb --build "$PKGDIR" "$OUT" >/dev/null
echo "build-deb: $OUT"
